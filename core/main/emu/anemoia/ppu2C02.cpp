...
void Ppu2C02::setPalette(uint8_t palette)
{
    ...
    case PALETTE_INDEX: {
        // Build (or reuse) an identity 8x64 table once: every emphasis row
        // simply returns the palette index. Lazily initialised on first use.
        static uint16_t identity_palette[8][64];
        static bool identity_inited = false;
        if (!identity_inited) {
            for (int e = 0; e < 8; ++e)
                for (int i = 0; i < 64; ++i)
                    identity_palette[e][i] = (uint16_t)i;
            identity_inited = true;
        }
        nes_palette = identity_palette;
        break;
    }
    default: break;
    }
